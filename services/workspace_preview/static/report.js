...
(() => {
 ...
 function render(data){window.WzosReportMap?.clear();$("report-content").replaceChildren();
   ...
   for(const form of data.forms){const d=node("details","");d.append(node("summary",`${form.title} · revision ${form.version} · ${form.status}`),node("p",form.details));if(form.safety){for(const [key,value] of Object.entries(form.safety))d.append(node("p",`${key.replaceAll("_"," ")}: ${value||"Not recorded"}`));}if(form.inspection){const v=form.inspection;d.append(node("p",`Vehicle ${v.vehicle_id} · ${v.trip_type} · Odometer: ${v.odometer??"not recorded"} miles`));for(const [key,label] of Object.entries({tires:"Tires",fluids:"Fluids",brakes:"Brakes",ebrake:"Emergency brake",mirrors:"Mirrors",windows:"Windows"}))d.append(node("p",`${label}: ${v[key].replaceAll("_"," ")}`));d.append(node("p",`Defects: ${v.defects||"None recorded"}`));}forms.append(d);}

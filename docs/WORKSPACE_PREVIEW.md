...
## Geometry and real conversation update (2026-09-21)

The measured-approach editor is now reused in this work-order form under **Work limits and measured approaches**. Save uses the same work-order revision/conflict checks. Add/remove up to eight approaches, preserve upstream-to-work-area coordinates and dated measurement sources. The backend rejects duplicate approach IDs, invalid paths and invalid scenario/closure combinations.

The question box now calls real local inference instead of keyword responses. Quick topic guides remain deterministic. See ATLAS_LOCAL_INTELLIGENCE.md for the two-process startup, model provenance, limits and verification. Chat uses saved data only, clears on identity/job changes and cannot execute app actions. Available status is checked when the panel opens. Final app branding: **WZOS powered by Atlas AI Assistant**. Earlier references to disconnected conversation or geometry-editor gaps describe prior checkpoints.
